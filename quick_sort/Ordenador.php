...
class Ordenador
{
    ...
    /**
     * O Quick Sort é um algoritmo de ordenação por divisão e conquista.
     * Ele escolhe um elemento como 'pivô' e particiona o array ao seu redor,
     * fazendo com que os elementos menores fiquem antes dele e os maiores depois.
     */
    public static function quickSort(array $array): array
    {
        self::executarQuickSort($array, 0, count($array) - 1);
        return $array;
    }

    // Método recursivo que manipula os índices (&$array é passado por referência)
    private static function executarQuickSort(array &$array, int $inicio, int $fim): void
    {
        if($inicio < $fim){
            // Faz o particionamento e descobre a posição correta do pivô
            $indicePivo = self::particionar($array, $inicio, $fim);

            // Ordena recursivamente a parte esquerda (do $inicio até $indicePivo - 1)
            self::executarQuickSort($array, $inicio, $indicePivo - 1);

            // Ordena recursivamente a parte direita (do $indicePivo + 1 até $fim)
            self::executarQuickSort($array, $indicePivo + 1, $fim);


        }
    }

    private static function particionar(array &$array, int $inicio, int $fim): int
    {
        $pivo = $array[$fim];               // Escolhe o último elemento como pivô
        $elementos_menores = $inicio -1;   // Ponteiro dos elementos menores

        for($i = $inicio; $i < $fim; $i++){
            // Se o elemento atual for menor ou igual ao pivô
            if($array[$i] <= $pivo){
                $elementos_menores++; // Avança o ponteiro dos menores

                // Realize a troca entre $array[$elementos_menores] e $array[$i] usando sua lógica do $auxiliar
                $auxiliar                  = $array[$elementos_menores];
                $array[$elementos_menores] = $array[$i];
                $array[$i]                 = $auxiliar;
            }
        }

        // Coloca o pivô na posição correta dele (logo após o último elemento menor)
        // Faça a troca entre $array[$elementos_menores + 1] e o pivô ($array[$fim])

        $auxiliar                      = $array[$elementos_menores + 1];
        $array[$elementos_menores + 1] = $array[$fim];
        $array[$fim]                   = $auxiliar;

        return $elementos_menores + 1; // Retorna o índice onde o pivô ficou fixado
    }
}
